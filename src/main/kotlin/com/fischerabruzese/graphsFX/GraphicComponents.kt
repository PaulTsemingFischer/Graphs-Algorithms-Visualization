...
class GraphicComponents<E : Any>(
    ...
    private fun ArrayList<Edge>.dumpPositions() =
        this.map { Position(((it.v2.x.get() + it.v1.x.get()) / 2.0), ((it.v2.y.get() + it.v1.y.get()) / 2.0)) }
            .toTypedArray()
        ...
        for ((v1pos, vertex1) in verticesElements.withIndex()) {
            for (vertex2 in verticesElements.subList(v1pos, verticesElements.size)) {
                ...
                if (v2tov1Weight > -1 || v1tov2Weight > -1)
                    edgeElements.add(
                        Edge(
                            graphicVertices[v1pos],
                            graphicVertices[verticesElements.indexOf(vertex2)],
                            v1tov2Weight,
                            v2tov1Weight
                        )
                    )
    ...
    inner class Vertex(val v: E, xInit: Double, yInit: Double) : StackPane() {
        ...
        var vTranslateXBinding: DoubleBinding =
            pane.widthProperty().multiply(this.x).multiply(usablePercentPaneWidth).add(Companion.CIRCLE_RADIUS)
        var vTranslateYBinding: DoubleBinding =
            pane.heightProperty().multiply(this.y).multiply(usablePercentPaneHeight).add(Companion.CIRCLE_RADIUS)
                ...
                while (!Thread.interrupted()) {
                    try {
                        pushGhostFrame(
                            generateFrame(
                                speed,
                                unaffected = listOfNotNull(selectedVertex),
                ...
                while (!Thread.interrupted()) {
                    ...
                    Platform.runLater {
                        try {
                            ...
                            when (ex) {
                                is NoSuchElementException, is IndexOutOfBoundsException -> {
                                    if (!stopping) { //if unexpected crash
                                        stopSimulation()
                                        startSimulation()
                                    }
    ...
    val physicsC = object : Physics(0.0) {
        ...
        override fun generateFrame(
            speed: Double,
            unaffected: List<GraphicComponents<E>.Vertex>,
            uneffectors: List<GraphicComponents<E>.Vertex>,
            verticesPos: List<Pair<GraphicComponents<E>.Vertex, Position>>
        ): Array<Displacement> {
            val max = 1500
            ...
            for ((affectedVertex, affectedPos) in verticesPos) {
                ...
                //vertices
                val (effectorVerts, effectorPos) = verticesPos.filterNot { (uneffectors.contains(it.first) || affectedVertex === it.first) }
                    .unzip()
                effectorVerts
